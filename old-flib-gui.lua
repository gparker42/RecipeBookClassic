if ... ~= "old-flib-gui" then
  return require("old-flib-gui")
end
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.hook_events(callback)
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.read_action(event_data)
  ...
  local elem_actions = mod_tags.old_flib
...
local function recursive_build(parent, structure, refs)
  ...
  local tags_flib = tags.old_flib
  tags.old_flib = actions
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.build(parent, structures)
  local refs = {}
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.add(parent, structure)
...
local function recursive_update(elem, updates)
  ...
  if updates.tags then
    old_flib_gui.update_tags(elem, updates.tags)
  end

  -- TODO: This could be a lot better
  if updates.actions then
    for event_name, payload in pairs(updates.actions) do
      old_flib_gui.set_action(elem, event_name, payload)
    end
  end
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.update(elem, updates)
  recursive_update(elem, updates)
end

--- @deprecated use `gui-lite` instead
function old_flib_gui.get_tags(elem)
  return elem.tags[mod_name] or {}
end

--- @deprecated use `gui-lite` instead
function old_flib_gui.set_tags(elem, tags)
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.delete_tags(elem)
  local elem_tags = elem.tags
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.update_tags(elem, updates)
  local elem_tags = elem.tags
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.set_action(elem, event_name, msg)
  local elem_tags = elem.tags
  local existing = elem_tags[mod_name]
  ...
  local actions = existing.old_flib
  if not actions then
    actions = {}
    existing.old_flib = actions
  end
...
--- @deprecated use `gui-lite` instead
function old_flib_gui.get_action(elem, event_name)
  local elem_tags = elem.tags
  local existing = elem_tags[mod_name]
